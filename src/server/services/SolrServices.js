...
/**
 * Custom solr query supporting grouping of results for advanced usages.
 * Grouping helps us reduce the number of api calls to be made to R&R service.
 * Read more about grouping in docs.
 *
 * Put rows as a large number to be safe. For 10 articles, you need 10 rows,
 * maybe increase to 100 or greater just to be safe.
 *
 * @param {String} id cluster id
 * @param {Object} queryObject json solr query object, see docs
 * @param {Object} groupObject see grouping of results in docs
 * @param {Object} sortObject solr sort object
 * @param {Number} rows no of rows to have in result, see solr docs.
 */

function customQuery(id, queryObject, groupObject, sortObject, rows = 10) {
  return new Promise((resolve, reject) => {
    const params = {
      cluster_id: id || config.SOLR_CLUSTER_ID,
      collection_name: 'solr_collection',
      wt: 'json',
    };
