...
const locations = require('../../../config/sales-mock-data/location');
const saleArticles = require('../../../config/sales-mock-data/saleArticles');
...
function generateDummySales(article, weatherInfo) {
  ...
  const newSale = avgSaleRate +
    ((weatherEffect) * (multiplier) * (Math.random() * avgSaleRate));
      ...
      _.forEach(locations.dummyLocations, (location) => {
        _.forEach(dates, (date) => {
          ...
          const data = {
            ...
            /**
             * TODO: Solr throws error for queries with spaces. Need to figure out why.
             */
            location_name: location.name.split(' ').join(''),
            ...
            // Read more about groupId in docs. It has a special use for grouping
            // the results to reduce api calls number.
            groupId: `${location.name}**${article.name}`,
          };
